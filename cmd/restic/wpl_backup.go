...
`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		return RunResticBackup(cmd.Context(), cmd, globalOptions, args)
	},
}
...
// the run restic backup command_args and other subcommands
func RunResticBackup(ctx context.Context, cmd *cobra.Command, gopts GlobalOptions, args []string) error {

	gOptions = gopts

	err := step_backup(ctx, gopts, cmd)
	if err != nil {
...
func step_backup(ctx context.Context, gopts GlobalOptions, cmd *cobra.Command) error {
	var (
		err         error
	...
	for _, logical_backup := range filesystem_to_backup {
		...
		repo_basepath := json_config["BASE_CONFIG"]["new-PC"]
      ...
      "--password-file", "/root/backup_password2",
		}
