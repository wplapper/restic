...
=======
  ...
  - Timing: give some timings of the various subsections of code
	- All   : do the run of all well known repositories
...
`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		return runTRemove(cmd.Context(), globalOptions, args)
	},
}
...
func init() {
	...
	f.BoolVarP(&tremoveOptions.All, "all", "A", false, "run for all known repositories")
}

func runTRemove(ctx context.Context, gopts GlobalOptions, args []string) error {
	if tremoveOptions.All {
		tremoveOptions.minimal = true
		for _, repoName := range WplRepositoryList {
			...
			gopts.Repo = repoName
			err := runOneRemove(ctx, gopts, args)
			if err != nil {
				continue
			}
		}
	} else {
		return runOneRemove(ctx, gopts, args)
	}
	return nil
}

func runOneRemove(ctx context.Context, gopts GlobalOptions, args []string) error {
...
func CalculatePruneSize(selected []SnapshotWpl, repositoryData *RepositoryData,
	...
	unused_blobs.Remove(EMPTY_NODE_ID_TRANSLATED)
	...
	for blob := range unused_blobs.Iter() {
		...
		if data_sett, ok := full_map[blob_ID]; ok {
			...
			if ih.Type == restic.DataBlob {
				...
				// BUG: the size assignment is wrong!
				raw_blob = BlobInfo{blob: blob, Type: ih.Type, size: ih.UncompressedLength,
...
func DoOneRepositoryWithSnaps(ctx context.Context, gopts GlobalOptions,
	...
	// check for existence in allSnapsShort
