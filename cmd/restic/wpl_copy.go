...
func RunResticCopy(ctx context.Context, cmd *cobra.Command, gopts GlobalOptions, args []string) error {
  ...
  var err error
  gOptions = gopts
  //secondaryGopts, _, err := fillSecondaryGlobalOpts(copy_filesys_opts.secondaryRepoOptions, gopts, "destination")
  secondaryGopts, _, err := fillSecondaryGlobalOpts(ctx, copy_filesys_opts.secondaryRepoOptions, gopts, "destination")
  if err != nil {
