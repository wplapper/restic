...
func TestRewriteInclude(t *testing.T) {
	...
	opts := RewriteOptions{
		IncludePatternOptions: filter.IncludePatternOptions{
			Includes: []string{"testfile"},
		},
	}
	rtest.OK(t, runRewrite(context.TODO(), opts, env.gopts, []string{snap.String()}))
	newSnapshotIDs := testRunList(t, "snapshots", env.gopts)
	rtest.Assert(t, len(newSnapshotIDs) == 2, "expected two snapshots, got %v", len(newSnapshotIDs))
	newSnap := restic.ID{}
	if newSnapshotIDs[0] == snap {
		newSnap = newSnapshotIDs[1]
	} else {
		newSnap = newSnapshotIDs[0]
	}

	env.gopts.JSON = true
	buf := testRunLsWithOpts(t, env.gopts, LsOptions{}, []string{string(newSnap.String())})
	_ = buf
	matches := []lsOut{}
	_ = matches
